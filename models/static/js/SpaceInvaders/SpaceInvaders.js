...
const audioJogo = new Audio("/static/audio/SpaceInvaders/justiceleague.mp3");
...
const velocidadeNave = 25;
const velocidadeTiro = 22;
const velocidadeNaveInimigas = 3.5;

let tirosDisponiveis = 30; //Começa com 30 tiros
let recarregando = false; 
let recargaTimeout; 

// Atualiza a exibição de recarga ou quantidade de tiros disponíveis
const atualizaRecarga = () => {
  if (recarregando) {
      document.getElementById("vida").textContent = `Vida: 100`; // Mantém a vida visível
      document.getElementById("tempoRecarga").textContent = "Recarregando..."; // Exibe o status de recarga
    
  } else {
      
      document.getElementById("tempoRecarga").textContent = `Tiros Disponíveis: ${tirosDisponiveis}`; // Exibe o número de tiros restantes
  }
}
...
const atirar = () => {
  if(recarregando) return; //Bloqueia atirar durante a recarga

  const delayTiro = Date.now();
  const atrasoTiro = delayTiro - tiroAtual;

  if(estaAtirando && atrasoTiro >= 100 & tirosDisponiveis > 0){
    tiroAtual = Date.now();
    tirosDisponiveis--
    atualizaRecarga()
...
const audioTiros = () => {
  ...
  audioDoTiro.setAttribute("src", "/static/audio/SpaceInvaders/tiro.mp3");
  audioDoTiro.play();
...
const naveInimigas = () => {
  ...
  inimigo.style.backgroundImage = "url(/static/img/SpaceInvaders/inimigo.gif)";
...
const moveNaveInimigas = () => {
  ...
  for (let i = 0; i < navaInimigas.length; i++) {
    if (navaInimigas[i]) {
      ...
      if (posicaoTopNaveInimiga > alturaCenario) { // Quando a nave inimiga passa da tela
        vidaAtual -= 5; // Reduz a vida
        vida.textContent = `Vida: ${vidaAtual}`; // Atualiza o texto da vida

        if (vidaAtual <= 0) { 
          gameOver(); // Chama o fim de jogo se a vida chegar a zero
        }
        
        explosaoNaveInimigaDestruida(posicaoLeftNaveInimiga);
        navaInimigas[i].remove(); // Remove a nave inimiga quando ela passa da tela
      } 
    }
  }
}
...
const naveInimigaDestruida = (posicaoLeftNaveInimiga, posicaoTopNaveInimiga) => {
  ...
  naveInimigaDestruida.style.backgroundImage = "url(/static/img/SpaceInvaders/eliminado.gif)";
...
const explosaoNaveInimigaDestruida = (posicaoLeftNaveInimiga) => {
  ...
  explosaoNaveInimiga.style.backgroundImage = "url(/static/img/SpaceInvaders/explosao.gif)";
...
const audioExplosoes = () => {
  ...
  audioExplosaoNaveInimiga.setAttribute("src", "/static/audio/SpaceInvaders/destruido.mp3");
